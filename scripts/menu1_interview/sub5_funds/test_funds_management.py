...
class TestFundsManagement:
    ...
    def test_add(self, text, num):
        ...
        time.sleep(1)
        self.page.input_text(path='新增/编辑-输入', content=text, param='资金名称', itype="clickinput")
        time.sleep(1)
        self.page.input_text(path='新增/编辑-输入', content=num, param='金额', itype="clickinput")
    ...
    def test_update(self, text, num):
        ...
        time.sleep(1)
        self.page.input_text(path='新增/编辑-输入', content=text, param='资金名称', itype="clearallinput")
        time.sleep(1)
        self.page.input_text(path='新增/编辑-输入', content=num, param='金额', itype="clearallinput")
        self.page.click_btn(path='侧边弹窗底部按钮', param='保存')
        assert self.page.sub_menu_alert()
