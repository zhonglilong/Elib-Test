...
class AdvanceOrderPage(BasePage):
    ...
    def sub_menu(self):
        """ 点击子菜单: 预订单"""
        self.click_btn(path='子菜单', param=[3, '预订单'])
    ...
    def input_filter_items(self, start, end):
        """ 获取 请输入起始订购号 和 请输入结束订购号，输入值
        :param start: 请输入起始订购号
        :param end: 请输入结束订购号
        """
        self.input_text(path='筛选-输入框', param='请输入起始订购号', content=start)
        self.input_text(path='筛选-输入框', param='请输入结束订购号', content=end)
